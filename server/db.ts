...
export async function getCalendarData(
  ...
  const topicIds = [...new Set(revisions.map((r: any) => r.topic_id))];
...
export async function checkExamIntegrated(id: any, userId?: any) {
  return false;
}
...
export async function exportDatabase() {
  return JSON.stringify({ message: "Dados no Supabase." });
}
...
export async function createFlashcard(data: any) {
  const { data: fc, error = null } = await supabase
    .from("flashcards")
    .insert({
      user_id: data.userId,
      topic_id: data.topicId,
...
export async function saveQuestionErrorSimilarQuestions(
  ...
  await supabase
    .from("question_errors")
    .update({ similar_questions: questions })
    .eq("id", id)
    .eq("user_id", userId);
}
...
export async function markQuestionErrorFlashcardGenerated(
  ...
  await supabase
    .from("question_errors")
    .update({ flashcard_generated: true })
    .eq("id", id)
    .eq("user_id", userId);
}
...
export async function saveEssay(data: any) {
  const { data: essay, error } = await supabase
    .from("study_notes")
    .insert({ ...data, user_id: data.userId, is_essay: true })
    .select()
    .single();
  return essay;
}
...
export async function logStudyEndTime(
  ...
  await updateUserSettings(userId, {
    lastStudyEndTime: { endHour, alertIssued },
  });
}
