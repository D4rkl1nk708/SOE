...
${fullText.substring(0, 25000)}`;
        ...
        const rawAiResponse = await callAiProvider(input.provider, input.apiKey, prompt, 8192);
        const questions = extractJSON(rawAiResponse);

        const storagePath = path.join(process.cwd(), "data", "mined_exams");
        if (!fs.existsSync(storagePath)) fs.mkdirSync(storagePath, { recursive: true });

        const safeName = input.fileName.replace(".pdf", "").replace(/[^a-z0-9]/gi, "_");
        const outputFileName = `questoes_${safeName}_${Date.now()}.json`;
        fs.writeFileSync(path.join(storagePath, outputFileName), JSON.stringify(questions, null, 2));

        return { success: true, count: questions.length, fileName: outputFileName };
      } catch (err: any) {
  ...
  searchOnlineExams: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const query = `${input.banca} ${input.cargo} ${input.ano} prova pdf download`.replace(/ /g, "+");
        ...
        const linkRegex = /<a href="\/url\?q=([^&"]+\.pdf[^&"]*|[^&"]+provas[^&"]*)/gi;
        let match;
        ...
        while ((match = linkRegex.exec(html)) !== null) {
          ...
          if (!uniqueLinks.has(url) && url.startsWith("http")) {
            ...
            let title = url.split("/").pop()?.replace(/(-|_|\.pdf)/g, " ").toUpperCase() || "Prova Localizada";
        ...
        if (results.length === 0) {
           ...
           const ddgRes = await fetch(ddgUrl, { headers: { "User-Agent": USER_AGENT } });
           const ddgHtml = await ddgRes.text();
           const ddgRegex = /href="([^"]+\.pdf|[^"]+provas[^"]+)"[^>]*>([\s\S]*?)<\/a>/gi;
           let ddgMatch;
           while ((ddgMatch = ddgRegex.exec(ddgHtml)) !== null) {
             ...
             if (!uniqueLinks.has(url) && url.startsWith("http")) {
               uniqueLinks.add(url);
               results.push({ title: ddgMatch[2].replace(/<[^>]*>?/gm, "").trim().substring(0, 60), url });
             }
           }
        }
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (!input.url.toLowerCase().endsWith(".pdf")) {
          const detailRes = await fetch(input.url, { headers: { "User-Agent": USER_AGENT } });
          ...
          if (match) {
            ...
            if (fallbackMatch) {
               ...
               finalPdfUrl = fallbackMatch[1].startsWith("http") ? fallbackMatch[1] : baseUrl + fallbackMatch[1];
            } else {
               throw new Error("Não encontrei um link direto de PDF nesta página. Tente outro resultado.");
            }
          }
        }

        // Passo Final: Baixar o PDF
        const pdfRes = await fetch(finalPdfUrl, { headers: { "User-Agent": USER_AGENT } });
        if (!pdfRes.ok) throw new Error("O link do PDF parece estar protegido ou offline.");
  ...
  integrateExam: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const q of questions) {
        ...
        let disc = disciplines.find(d => d.name.toLowerCase() === subjectName.toLowerCase());
        if (!disc) {
          const { id } = await db.createDiscipline({ userId, name: subjectName, color: "var(--primary)", weight: 1 });
          disc = { id } as any;
        }

        const topics = await db.getTopicsByUser(userId, { disciplineId: disc.id });
        const topicName = q.topic || "Geral";
        let top = topics.find(t => t.name.toLowerCase() === topicName.toLowerCase());
        if (!top) {
          const { id } = await db.createTopic({ userId, disciplineId: disc.id, name: topicName, studyDate: new Date().toISOString(), notes: `Minerado: ${contestId}` });
          top = { id } as any;
        }

        await db.saveQuestionError({
          userId, topicId: top.id, disciplineId: disc.id,
          banca: "IA", contest: contestId, statement: q.statement,
          supportText: q.supportText,
          alternatives: Object.entries(q.alternatives || {}).map(([letter, text]) => ({ letter, text: String(text) })),
          correctAnswer: q.correctAnswer, createdAt: new Date().toISOString(),
          source: "mined"
        });
      }
      return { success: true };
    ...
    .query(async ({ ctx }) => {
      ...
      for (const f of files) {
        ...
        if (await db.checkExamIntegrated(contestId, ctx.user.id)) {
          const content = JSON.parse(fs.readFileSync(path.join(storagePath, f), "utf-8"));
          const questionsWithMeta = content.map((q: any, idx: number) => ({
            ...
            alternatives: Array.isArray(q.alternatives) 
              ? q.alternatives 
              : Object.entries(q.alternatives || {}).map(([letter, text]) => ({ letter, text: String(text) }))
          }));
  ...
  analyzeBancaTrend: protectedProcedure
    .input(z.object({ fileNames: z.array(z.string()), apiKey: z.string(), provider: z.string() }))
    .mutation(async ({ input }) => {
      const storagePath = path.join(process.cwd(), "data", "mined_exams");
      let combinedContent = "";
      ...
      const analysis = await callAiProvider(input.provider as any, input.apiKey, prompt, 4000);
      return { analysis };
    }),

  mapToEdital: protectedProcedure
    .input(z.object({ fileName: z.string(), apiKey: z.string(), provider: z.string() }))
    .mutation(async ({ input, ctx }) => {
      const db = await import("./db");
      const storagePath = path.join(process.cwd(), "data", "mined_exams");
      const questions = JSON.parse(fs.readFileSync(path.join(storagePath, input.fileName), "utf-8"));
      const topics = await db.getTopicsByUser(ctx.user.id, {});
      const topicList = topics.map(t => t.name).join(", ");
      const prompt = `Cruze as questões desta prova com os tópicos do meu Edital.\nQUESTÕES: ${questions.map((q: any) => q.statement).join(" | ").substring(0, 5000)}\nMEU EDITAL: ${topicList}`;
      const mappingRaw = await callAiProvider(input.provider as any, input.apiKey, prompt, 2000);
  ...
  getJson: protectedProcedure
    .input(z.object({ fileName: z.string() }))
    .query(async ({ input }) => {
      const filePath = path.join(process.cwd(), "data", "mined_exams", input.fileName);
