...
${questionText}`;
        ...
        const callGeminiWithFallback = async (
          ...
          throw new Error(
            `Nenhum modelo Gemini disponível. Último erro: ${lastError}`,
          );
        };
        ...
        if (user && topicObj && responseText.length > 30) {
          try {
            ...
            if (!currentNotes.includes("💡 Dica do Mentor")) {
              await storage.updateTopic(topicObj.id, user.id, {
  ...
  app.post("/api/tec/banca-increment", async (req: any, res: any) => {
    try {
      ...
      if (!disc)
        return res.json({ ok: false, error: "Disciplina não encontrada" });
      ...
      if (!topic)
        return res.json({ ok: false, error: "Assunto não encontrado" });
      const prev = topic.performance?.bancaStats || {};
      const bp = prev[banca] || { correct: 0, wrong: 0 };
      const bancaStats = {
        ...prev,
        [banca]: {
          correct: bp.correct + (correctAdd || 0),
          wrong: bp.wrong + (errorAdd || 0),
        },
      };
      ...
      await storage.updateUserSettings(user.id, {
        bancaStats: {
          ...gb,
          [banca]: {
            correct: gbEntry.correct + (correctAdd || 0),
            wrong: gbEntry.wrong + (errorAdd || 0),
          },
        },
      });
      res.json({ ok: true });
