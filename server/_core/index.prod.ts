...
async function startServer() {
  ...
  app.use("/api/tec", (req: any, res: any, next: any) => {
    ...
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, Accept, X-SOE-Token");
    if (req.method === "OPTIONS") { res.sendStatus(204); return; }
    next();
  });
  ...
  app.get("/api/dou-proxy", async (req: any, res: any) => {
    try {
      ...
      if (!ajaxResp.ok) {
        return res.status(ajaxResp.status).json({ error: `DOU retornou ${ajaxResp.status}` });
      }
    ...
    try {
      ...
      for (const url of endpoints) {
        try {
          ...
          const dataIdPattern = /data-id="([^"]+)"[^>]*>[\s\S]*?<[^>]*class="[^"]*title[^"]*"[^>]*>([\s\S]*?)<\/[^>]*>/gi;
          let m;
          while ((m = dataIdPattern.exec(html)) !== null) {
            results.push({ id: m[1], title: m[2].replace(/<[^>]+>/g, "").trim() });
          }
          ...
          while ((m = linkPattern.exec(html)) !== null) {
            ...
            if (title.length > 5) {
              results.push({ id: m[1], title, url: `https://www.in.gov.br${m[1]}` });
            }
          }
          ...
          res.setHeader("Content-Type", "application/json; charset=utf-8");
          res.setHeader("Access-Control-Allow-Origin", "*");
          return res.json({ source: "html-parsed", results, total, rawLength: html.length });
        } catch (err: any) {
      ...
      return res.status(502).json({ error: `Todos os endpoints falharam: ${lastError}` });
    } catch (e: any) {
      res.status(500).json({ error: String(e?.message ?? e) });
    }
  });
  ...
  app.post("/api/tec/caderno-push", async (req: any, res: any) => {
    try {
      ...
      if (!token) return res.status(401).json({ error: "Header X-SOE-Token ausente ou query ?token= não informado" });

      const user = await storage.getUserByPushToken(token);
      if (!user) return res.status(401).json({ error: "Token inválido. Gere um novo token em: Mentor → Cadernos Tempo Real → Gerar Token." });

      const { cadernoId, cadernoUrl, disciplina: disciplinaHint, rows } = req.body as {
        cadernoId: string;
        cadernoUrl: string;
        disciplina: string;
        rows: Array<{
          disciplina: string; assunto: string; acertos: number; erros: number; total?: number;
          incidencia?: number; totalQuestoesBanca?: number; bancaDominante?: string; dificuldade?: number;
        }>;
      };

      if (!rows || !Array.isArray(rows) || rows.length === 0) {
        return res.status(400).json({ error: "Nenhum dado recebido no campo 'rows'" });
      }
      ...
      function calculateRevisionDatesLocal(studyDate: Date, p: any) {
        ...
        for (let i = 1; i <= 5; i++) { cur = new Date(cur.getTime() + ms(p.revisionIntervalDays)); revisions.push({ date: new Date(cur), revisionNumber: i, type: "revision" }); }
        for (let i = 6; i <= 15; i++) { cur = new Date(cur.getTime() + ms(p.revisionSecondPhaseDays)); revisions.push({ date: new Date(cur), revisionNumber: i, type: "revision" }); }
        return revisions;
      }

      function generateTestsLocal(studyDate: Date, revisions: any[], intervalDays: number) {
        const tests: any[] = [];
        ...
        for (const rev of revisions) {
          ...
          while (cur.getTime() < rev.date.getTime() - 2 * ms(1)) {
            tests.push({ date: new Date(cur), revisionNumber: n++, type: "test" });
            cur = new Date(cur.getTime() + (intervalDays + Math.floor(Math.random() * Math.max(1, intervalDays))) * ms(1));
          }
          prev = rev.date;
        }
        return tests;
      ...
      const topicsWithNewErrors: { topicId: number; topicName: string; newErrors: number }[] = [];

      for (const row of rows) {
        const discName = (row.disciplina || disciplinaHint || "TEC Concursos").trim();
        ...
        let disc = disciplines.find(d => {
          ...
          return nd === normDisc || nd.includes(normDisc) || normDisc.includes(nd);
        });
        if (!disc) {
          const { id } = await storage.createDiscipline({ userId: user.id, name: discName, color: "#6366f1", weight: 1 });
        ...
        if (topic) {
          ...
          await storage.setTopicPerformance(topic.id, user.id, {
            correctCount: row.acertos, errorCount: row.erros,
            ...(row.incidencia !== undefined ? { incidencia: row.incidencia > 1 ? row.incidencia / 100 : row.incidencia } : {}),
            ...(row.totalQuestoesBanca !== undefined ? { totalQuestoesBanca: row.totalQuestoesBanca } : {}),
            ...(row.bancaDominante ? { bancaDominante: row.bancaDominante } : {}),
            ...(row.dificuldade !== undefined ? { dificuldade: row.dificuldade } : {}),
          });
          ...
          const { id: topicId } = await storage.createTopic({
            userId: user.id, disciplineId: disc!.id, name: assuntoName, studyDate, notes: null,
          });
          ...
          const tests = generateTestsLocal(studyDateObj, revisions, params.testIntervalDays);
          const allActs = [...revisions, ...tests].sort((a, b) => a.date.getTime() - b.date.getTime());
          await storage.createRevisions(allActs.map(a => ({
      ...
      const snapshotTopics = allTopicsNow
        ...
        .map(t => ({
          topicName: t.name,
          disciplineName: allDiscsNow.find(d => d.id === t.disciplineId)?.name ?? "Desconhecida",
...
// Tracker de alertas (User -> Topic -> Timestamps de erros)
  ...
  app.post("/api/tec/increment", express.json({ limit: "2mb" }), async (req: any, res: any) => {
    try {
      ...
      let disc = disciplines.find(d => {
        const nd = normalize(d.name);
        return nd === normDisc || nd.includes(normDisc) || normDisc.includes(nd) || (nd.length > 5 && normDisc.includes(nd)) || (normDisc.length > 5 && nd.includes(normDisc));
      });

      if (!disc) {
        const created = await storage.createDiscipline({ userId: user.id, name: disciplinaName, color: "#6366f1", weight: 1 });
      ...
      if (timeSpentSeconds && timeSpentSeconds > 0) {
         await storage.updateDiscipline(disc!.id, user.id, { studyTimeSeconds: (disc!.studyTimeSeconds || 0) + timeSpentSeconds });
      }

      let topics = await storage.getTopicsByUser(user.id);
      const normAssunto = normalize(assuntoName);
      let topic = topics.find(t => t.disciplineId === disc!.id && normalize(t.name) === normAssunto);
      let topicId = 0;
      ...
      if (topic) {
        ...
        await storage.setTopicPerformance(topic.id, user.id, { correctCount: newCorrect, errorCount: newErrors });
        if (timeSpentSeconds && timeSpentSeconds > 0) {
           await storage.updateTopic(topic.id, user.id, { studyTimeSeconds: (topic.studyTimeSeconds || 0) + timeSpentSeconds });
        }
      } else {
        ...
        topicId = created.id;
        await storage.setTopicPerformance(created.id, user.id, { correctCount: correctAdd || 0, errorCount: errorAdd || 0 });
        if (timeSpentSeconds && timeSpentSeconds > 0) {
           await storage.updateTopic(created.id, user.id, { studyTimeSeconds: timeSpentSeconds });
        }
      }
      ...
      res.json({ success: true, message: "Estatística incrementada com sucesso", blockPage, alertMessage });
    } catch (e: any) {
   ...
   */
  app.post("/api/tec/wrong-question", express.json({ limit: "2mb" }), async (req: any, res: any) => {
    try {
      const token = (req.headers["x-soe-token"] as string) || (req.query.token as string);
      if (!token) return res.status(401).json({ error: "Token ausente" });
      ...
      const {
        questionId, banca, year, contest, statement,
        alternatives, correctAnswer, userAnswer, resolution,
        disciplina: disciplinaName, assunto: assuntoName
      } = req.body;
      ...
      if (disciplinaName) {
        ...
        if (!disc) {
          const created = await storage.createDiscipline({ userId: user.id, name: disciplinaName, color: "#e74c3c", weight: 1 });
          discId = created.id;
          disciplines = await storage.getDisciplinesByUser(user.id);
        } else {
          discId = disc.id;
        ...
        if (assuntoName && discId) {
          const normAssunto = normalize(assuntoName);
          let topics = await storage.getTopicsByUser(user.id);
          let topic = topics.find(t => t.disciplineId === discId && (normalize(t.name) === normAssunto || (normalize(t.name).length > 10 && normAssunto.includes(normalize(t.name))) || (normAssunto.length > 10 && normalize(t.name).includes(normAssunto))));
          if (!topic) {
            const created = await storage.createTopic({
              userId: user.id, disciplineId: discId, name: assuntoName, studyDate: new Date().toISOString(), notes: null,
            });
            topId = created.id;
          } else {
            topId = topic.id;
      ...
      await storage.saveQuestionError({
        ...
        questionId,
        banca,
        year: typeof year === 'number' ? year : parseInt(year, 10) || undefined,
        contest,
        statement,
      ...
      res.json({ success: true, message: "Questão salva em Questões Erradas!" });
    } catch (e: any) {
   ...
   */
  app.post("/api/tec/generate-flashcard", express.json({ limit: "2mb" }), async (req: any, res: any) => {
    try {
      const token = (req.headers["x-soe-token"] as string) || (req.query.token as string);
      if (!token) return res.status(401).json({ error: "Token ausente" });
      const user = await storage.getUserByPushToken(token);
      ...
      if (!questionId) return res.status(400).json({ error: "ID da questão ausente" });

      const errors = await storage.getQuestionErrorsByUser(user.id);
      const e = errors.find((err: any) => err.questionId === questionId);
      if (!e) return res.status(404).json({ error: "Questão não encontrada no banco de erros." });

      const userKey = user?.settings?.aiApiKey;
      const userProv = user?.settings?.aiProvider || "gemini";
      const apiKey = userKey || process.env.GEMINI_API_KEY || process.env.OPENAI_API_KEY;
      const finalProvider = userKey ? userProv : (process.env.GEMINI_API_KEY ? "gemini" : "openai");
...
{"front": "pergunta direta e objetiva (máx 2 linhas)", "back": "resposta clara com a regra fundamental (máx 3 linhas)"}`;
        ...
        for (const model of GEMINI_MODELS) {
          try {
            const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`, {
              ...
              body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { maxOutputTokens: 512, temperature: 0.4 } }),
          ...
          } catch (e) { console.warn(`Flashcard: falha no modelo ${model}`, e); }
        }
      } else if (finalProvider === "openai") {
        const r = await fetch("https://api.openai.com/v1/chat/completions", {
          method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
          body: JSON.stringify({ model: "gpt-4o-mini", messages: [{ role: "user", content: prompt }], max_tokens: 512, temperature: 0.4 }),
          signal: AbortSignal.timeout(30000)
        });
  ...
  app.post("/api/tec/ai-mentor", express.json({ limit: "2mb" }), async (req: any, res: any) => {
    try {
      const token = (req.headers["x-soe-token"] as string) || (req.query.token as string);
      let user = null;
      if (token) user = await storage.getUserByPushToken(token);

      const { questionText, subject, fatigue, discipline } = req.body;
      if (!questionText) return res.status(400).json({ error: "Texto da questão ausente" });
      ...
      if (!apiKey) {
         return res.status(500).json({ error: "❌ Chave de API da IA não configurada. Por favor, adicione sua chave Gemini ou OpenAI na aba Perfil > Configurações de IA no SOE Desktop." });
      }
      ...
      if (user && subject && discipline) {
         ...
         if (disc) {
            ...
            topicObj = topics.find(t => t.disciplineId === disc.id && normalize(t.name) === normAss);
      ...
      const prompt = `Você é o "Mentor SOE", o Assistente de Concursos. 
	O aluno está preso nesta questão. Seja SOCRÁTICO e DIRETO. NÃO dê o gabarito.
	
	REGRAS:
	1. NUNCA comece com saudações (ex: "Olá", "Vejo que se dedicou").
	2. Vá direto ao ponto técnico ou à dica de sobrevivência (regra, mnemônico ou provocação sobre um termo chave).
	3. Seja completo e didático. Sem limite de palavras, escreva o quanto for necessário para o entendimento total. Quebre em parágrafos com markdown (* e **).
${fatiguePrompt}
${survivalNotes ? `\nAnotações atuais do aluno sobre o tema:\n"${survivalNotes.substring(0, 500)}"\nConsidere usar o resumo dele.` : ""}
...
${questionText}`;
      ...
      const callGeminiWithFallback = async (apiKey: string, prompt: string, maxOutputTokens = 2000): Promise<string> => {
        ...
        for (const model of GEMINI_MODELS_FALLBACK) {
          const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`, {
            ...
            body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { maxOutputTokens, temperature: 0.7 } }),
          });
          const d = await r.json() as any;
          if (d.error) {
            ...
            if (isQuota || isNotFound) { lastError = `[${model}] ${msg}`; console.warn(`[Gemini] Modelo ${model} indisponível, tentando próximo...`); continue; }
            throw new Error(msg);
          }
      ...
      let responseText = "";
      if (finalProvider === "gemini") {
	        responseText = await callGeminiWithFallback(apiKey, prompt, 4096) || "A IA não conseguiu gerar uma dica para esta questão.";
	      } else if (finalProvider === "openai") {
	        const r = await fetch("https://api.openai.com/v1/chat/completions", {
	          method: "POST",
	          ...
	          body: JSON.stringify({
	            model: "gpt-4o-mini",
	            messages: [{ role: "system", content: "Você é um mentor direto e técnico. Não use saudações." }, { role: "user", content: prompt }],
	            max_tokens: 4096,
	          }),
	        });
        const d = await r.json();
        if (d.error) throw new Error(d.error.message || "Erro OpenAI");
        responseText = d.choices?.[0]?.message?.content || "A IA não conseguiu gerar uma dica para esta questão.";
        ...
        responseText = d.content?.[0]?.text || "A IA não conseguiu gerar uma dica para esta questão.";
      }

      // Memória de longo prazo: Salvar ensinamento socrático no tópico
      if (user && topicObj && responseText.length > 30) {
         try {
            const shortAdvice = "\n\n💡 Dica do Mentor: " + responseText.replace(/\n/g, ' ').substring(0, 180) + "...";
            const currentNotes = topicObj.notes || "";
            if (!currentNotes.includes("💡 Dica do Mentor")) {
               await storage.updateTopic(topicObj.id, user.id, { notes: currentNotes + shortAdvice });
            }
         } catch(e) {}
      }
  ...
  app.post("/api/tec/cadernos-list", async (req: any, res: any) => {
    try {
      ...
      const { cadernos } = req.body as {
        cadernos: Array<{ id: string; nome: string; disciplina?: string; url: string; totalQuestoes?: number; assuntos?: number }>;
      };
      if (!Array.isArray(cadernos) || cadernos.length === 0)
        return res.status(400).json({ ok: false, error: "Lista de cadernos vazia" });
      ...
      for (const c of cadernos) {
        if (!existingIds.has(c.id)) {
          await storage.saveCadernoTec(user.id, {
            cadernoId: c.id, cadernoUrl: c.url,
            disciplina: c.disciplina || c.nome || "TEC Concursos",
            lastSync: new Date().toISOString(), topicsCount: c.assuntos || 0,
          });
          newCount++;
        }
      }
      res.json({ ok: true, total: cadernos.length, newCount, message: `${cadernos.length} caderno(s) detectado(s), ${newCount} novo(s) registrado(s).` });
    } catch (e: any) {
   ...
   * Incrementa stats de banca por assunto.
   */
  app.post("/api/tec/banca-increment", async (req: any, res: any) => {
    try {
      ...
      const { assunto, disciplina, banca, correctAdd, errorAdd } = req.body as {
        assunto: string; disciplina: string; banca: string; correctAdd: number; errorAdd: number;
      };
      if (!assunto || !banca) return res.status(400).json({ ok: false, error: "assunto e banca obrigatórios" });
      const normalize = (s: string) =>
        s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9\s]/g, "").replace(/\s+/g, " ").trim();
      const disciplines = await storage.getDisciplinesByUser(user.id);
      const topics = await storage.getTopicsByUser(user.id);
      const disc = disciplines.find((d) => { const nd = normalize(d.name); const ni = normalize(disciplina || ""); return nd === ni || nd.includes(ni) || ni.includes(nd); });
      if (!disc) return res.json({ ok: false, error: "Disciplina não encontrada" });
      const topic = topics.find((t) => t.disciplineId === disc.id && normalize(t.name) === normalize(assunto));
      if (!topic) return res.json({ ok: false, error: "Assunto não encontrado" });
      const prev = topic.performance?.bancaStats || {};
      const bp = prev[banca] || { correct: 0, wrong: 0 };
      const bancaStats = { ...prev, [banca]: { correct: bp.correct + (correctAdd || 0), wrong: bp.wrong + (errorAdd || 0) } };
      const bancaDominante = Object.entries(bancaStats).sort(([,a],[,b]) => (b.correct+b.wrong)-(a.correct+a.wrong))[0]?.[0] ?? banca;
      await storage.setTopicPerformance(topic.id, user.id, {
      ...
      const settings = await storage.getUserSettings(user.id);
      const gb = settings?.bancaStats || {};
      const gbEntry = gb[banca] || { correct: 0, wrong: 0 };
      await storage.updateUserSettings(user.id, { bancaStats: { ...gb, [banca]: { correct: gbEntry.correct + (correctAdd||0), wrong: gbEntry.wrong + (errorAdd||0) } } });
      res.json({ ok: true });
    } catch (e: any) {
  ...
  app.get("/api/tec/status", async (req: any, res: any) => {
    try {
      ...
      res.json({ ok: true, userName: user.name, cadernosCount: cadernos.length });
    } catch (e: any) {
  ...
  app.get("/api/ical/:token", async (req: any, res: any) => {
    try {
      ...
      const now_stamp = new Date().toISOString().replace(/[-:.]/g, "").slice(0, 15) + "Z";
      ...
      const events = toExport.map((r) => {
        ...
        const typeLabel = r.type === "test" ? "🧪 Teste" : `📖 Revisão ${r.revisionNumber}`;
        const summary = escIcs(`${typeLabel}: ${topic?.name ?? "Tema"} (${disc?.name ?? "Disciplina"})`);
      ...
      res.setHeader("Content-Disposition", "attachment; filename=soe-revisoes.ics");
  ...
  // ── Sync/export route (for QR code LAN sync) ──────────────────────────
  ...
  app.get("/api/sync/export", (_req, res) => {
    try {
      ...
      if (!fs.existsSync(dbFile)) return res.status(404).json({ error: "No database found" });
  ...
  app.post("/api/sync/import", (req, res) => {
    try {
      ...
      const dbFile = path.join(dataDir, "database.json");
      const body = req.body;
      // CapacitorHttp pode enviar JSON como string crua — aceitar os dois formatos
      const jsonStr = typeof body === "string" ? body : JSON.stringify(body, null, 2);
      if (!jsonStr || jsonStr === "null") return res.status(400).json({ error: "Invalid body" });
  ...
  // ── Receive from Android (Android → PC via QR Push) ───────────────────
  ...
  app.options("/api/sync/receive", (_req, res) => {
    res.setHeader("Access-Control-Allow-Origin", "*");
  ...
  app.post("/api/sync/receive", (req, res) => {
    try {
      ...
      if (!jsonStr || jsonStr === "null") return res.status(400).json({ error: "Invalid body" });
      fs.writeFileSync(dbFile, jsonStr, "utf-8");
      // Notifica o PC via SSE que os dados chegaram
      for (const client of receiveSSEClients) {
        try { client.write("data: received\n\n"); } catch { }
      }
      res.setHeader("Access-Control-Allow-Origin", "*");
      res.json({ success: true });
    } catch (e) {
  ...
  app.get("/api/backup/list", (_req, res) => {
    try {
      ...
      res.json({ backups: files.map(f => ({ name: f, date: f.replace("backup_", "").replace(".json", "") })) });
    } catch (e) {
      res.status(500).json({ error: String(e) });
    }
  });

  serveStatic(app, path.join(__dirname, "public"));
