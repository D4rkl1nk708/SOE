...
async function startServer() {
  ...
  app.post("/api/_debug_log", express.json(), (req: any, res: any) => {
    require('fs').appendFileSync('/tmp/soe_webview.log', JSON.stringify(req.body) + '\n');
    res.json({ ok: true });
  });
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (topic) {
          ...
          await storage.setTopicPerformance(topicId, user.id, { correctCount: row.acertos, errorCount: row.erros });
...
{"front": "pergunta direta e objetiva (máx 2 linhas)", "back": "resposta clara com a regra fundamental (máx 3 linhas)"}`;
  ...
  app.post("/api/tec/ai-mentor", express.json({ limit: "2mb" }), async (req: any, res: any) => {
    try {
      ...
      const { questionText, subject, fatigue, discipline, questionId } = req.body;
...
${questionText}`;
      ...
      const callGeminiWithFallback = async (apiKey: string, prompt: string, maxOutputTokens = 2000): Promise<string> => {
        ...
        throw new Error(`Cota Gemini esgotada em todos os modelos. ${lastError}`);
      };
      ...
      if (user && questionId) {
        try {
           const errors = await storage.getQuestionErrorsByUser(user.id);
           const match = errors.find((e: any) => e.questionId === questionId);
           if (match) {
              await storage.saveQuestionErrorAnalysis(match.id, user.id, responseText);
           }
        } catch(e) { }
      }
  ...
  app.post("/api/tec/banca-increment", async (req: any, res: any) => {
    try {
      ...
      const { assunto, disciplina, banca, correctAdd, errorAdd } = req.body as {
        assunto: string; disciplina: string; banca: string;
        correctAdd: number; errorAdd: number;
      };
      ...
      if (!disc) return res.json({ ok: false, error: "Disciplina não encontrada no SOE" });

      const topic = topics.find(
        (t) => t.disciplineId === disc.id && normalize(t.name) === normalize(assunto)
      );
      if (!topic) return res.json({ ok: false, error: "Assunto não encontrado no SOE" });
      ...
      // Find dominant banca
      const bancaDominante = Object.entries(bancaStats)
        .sort(([, a], [, b]) => (b.correct + b.wrong) - (a.correct + a.wrong))[0]?.[0] ?? banca;
      ...
      await storage.updateUserSettings(user.id, {
        bancaStats: { ...globalBanca, [banca]: { correct: gb.correct + (correctAdd || 0), wrong: gb.wrong + (errorAdd || 0) } },
      });
  ...
  app.post("/api/backup/auto", (_req, res) => {
    try {
      ...
      if (!fs.existsSync(dest)) {
        ...
        const files = fs.readdirSync(backupDir).filter((f: string) => f.startsWith("backup_")).sort();
        if (files.length > 30) {
  ...
  app.get("/api/backup/list", (_req, res) => {
    try {
      ...
      const files = fs.readdirSync(backupDir).filter((f: string) => f.startsWith("backup_")).sort().reverse();
      res.setHeader("Access-Control-Allow-Origin", "*");
      res.json({ backups: files.map((f: string) => ({ name: f, date: f.replace("backup_", "").replace(".json", "") })) });
    } catch (e) { res.status(500).json({ error: String(e) }); }
  });
