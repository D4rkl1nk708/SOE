...
import { 
  ...
  Loader2, Search, Download, Microscope, 
  AlertTriangle, Clock, XCircle, Database, ListChecks, Play, Trash2,
  Cpu, Zap, Share2, FlaskConical, Target, BarChart3, ChevronRight,
  Info
} from "lucide-react";
...
export default function Lab() {
  const [activeTab, setActiveTab] = useState<"mining" | "library" | "strategy">("mining");
  ...
  const { data: history, refetch: refetchHistory } = trpc.lab.listHistory.useQuery();
  ...
  const handleDownloadAndMine = async (url: string, title: string) => {
    try {
      ...
      const { base64, fileName } = await downloadUrlMutation.mutateAsync({ url, fileName: `${title}.pdf` });
  ...
  const handleToggleIntegration = async (fileName: string, currentStatus: boolean) => {
    try {
  ...
  const handleExportJson = async (fileName: string) => {
    try {
      const data = await utils.client.lab.getJson.fetch({ fileName });
  ...
  useEffect(() => {
    const processNext = async () => {
      ...
      if (!nextItem || queue.some(item => item.status === 'processing')) return;
      ...
      try {
        ...
        await processMutation.mutateAsync({ base64, fileName: nextItem.file.name, apiKey, provider });
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!apiKey) { toast.error("Configure sua API Key"); return; }
    setQueue(prev => [...prev, ...files.filter(f => f.type === "application/pdf").map(f => ({ id: Math.random().toString(36).substr(2, 9), file: f, status: 'pending' as const }))]);
  };

  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-20 pt-10 px-4">
      {showPlayer && <MinedResolutionPanel onClose={() => setShowPlayer(false)} />}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
        <div className="space-y-6">
           <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-secondary/50 border border-primary/20">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">SOE Intelligent Mining</span>
           </div>
           <h1 className="text-6xl font-black tracking-tighter leading-none">
              Laboratório de <span className="text-primary/80">Provas</span>
           </h1>
           <p className="text-muted-foreground text-sm max-w-xl opacity-60">Sua central privada de mineração e estruturação de dados acadêmicos com Inteligência Artificial.</p>
        </div>
          ...
          <div className="soe-card p-10 rounded-[4rem] bg-primary/5 border-primary/20 space-y-8">
             <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                <div className="space-y-2">
                   <label className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 ml-4">Banca Examinadora</label>
                   <input 
                    type="text" 
                    id="search-banca"
                ...
                <div className="md:col-span-2 space-y-2">
                   <label className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 ml-4">Cargo ou Área</label>
                   <input 
                    type="text" 
                    id="search-cargo"
                ...
                <div className="space-y-2">
                   <label className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 ml-4">Ano</label>
                   <select 
                    id="search-ano"
             ...
             <div className="flex items-center justify-between pt-4 border-t border-primary/10">
                <p className="text-[10px] text-muted-foreground opacity-60 italic flex items-center gap-2">
                   ...
                   A IA vasculha portais de concursos para encontrar PDFs oficiais.
                </p>
                <button 
                  onClick={async () => {
                    const banca = (document.getElementById('search-banca') as HTMLInputElement).value;
                    const cargo = (document.getElementById('search-cargo') as HTMLInputElement).value;
                    const ano = (document.getElementById('search-ano') as HTMLSelectElement).value;
                    ...
                    try {
                      const results = await searchOnlineMutation.mutateAsync({ banca, cargo, ano });
                      if (results && results.length > 0) {
                        setSearchResults(results as any);
                        toast.success(`Encontrei ${results.length} provas.`);
                      } else {
                        toast.error("Nenhuma prova encontrada com esses critérios.");
                      }
                    } catch (err: any) {
          ...
          {searchResults.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in zoom-in-95 duration-500">
               {searchResults.map((res, i) => (
                 <div key={i} className="soe-card p-6 flex items-center justify-between bg-secondary/30 border-primary/20 rounded-[2rem]">
                    <div className="flex items-center gap-4">
                       <div className="p-3 rounded-xl bg-primary/10 text-primary">
                          <FileJson size={20} />
                       </div>
                       <div className="space-y-0.5">
                          <h5 className="text-[11px] font-black uppercase tracking-tight truncate max-w-[200px]">{res.title}</h5>
                          <a 
                            href={res.url} 
                            target="_blank" 
                    ...
                    {downloadingUrls.has(res.url) ? (
                       <div className="flex flex-col items-end gap-2 px-6">
                          ...
                          <span className="text-[8px] font-black uppercase text-primary animate-pulse">Processando</span>
                       </div>
                    ) : (
                       <button 
          ...
          <div className="relative h-80 rounded-[4rem] border-2 border-dashed border-border overflow-hidden bg-secondary/5 group transition-all hover:border-primary/40">
             <div className="absolute inset-0 opacity-[0.03] grid grid-cols-10 gap-12 p-12 pointer-events-none select-none">
                {Array.from({ length: 60 }).map((_, i) => (
                  <div key={i} className="flex flex-col items-center gap-2">
                    {i % 4 === 0 ? <FileJson size={24} /> : i % 4 === 1 ? <Database size={24} /> : i % 4 === 2 ? <Microscope size={24} /> : <Zap size={24} />}
                  </div>
                ))}
             </div>
             ...
             <div className="relative h-full flex flex-col items-center justify-center gap-8">
                ...
                <div className="text-center space-y-3">
                   <h2 className="text-3xl font-black uppercase tracking-[0.25em] text-foreground">Arraste seus PDFs</h2>
                   <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground opacity-40 max-w-sm mx-auto">Suporta editais, cadernos e provas íntegras</p>
                </div>
             </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="soe-card p-10 flex items-center gap-8 bg-secondary/10 border-border/50 group hover:border-primary/30 transition-all rounded-[3rem]">
                ...
                <div className="space-y-1">
                   <h4 className="text-xs font-black uppercase tracking-widest">OCR Inteligente</h4>
                   <p className="text-[11px] text-muted-foreground opacity-50 leading-relaxed">Detecta automaticamente enunciados, alternativas e gabaritos em qualquer layout.</p>
                </div>
             </div>
             ...
             <div className="soe-card p-10 flex items-center gap-8 bg-secondary/10 border-border/50 group hover:border-primary/30 transition-all rounded-[3rem]">
                ...
                <div className="space-y-1">
                   <h4 className="text-xs font-black uppercase tracking-widest">Exportação JSON</h4>
                   <p className="text-[11px] text-muted-foreground opacity-50 leading-relaxed">Salve seus bancos minerados localmente ou exporte para integração privada.</p>
                </div>
             </div>
          </div>
          ...
          {queue.length > 0 && (
            <div className="soe-card p-10 space-y-8 animate-in zoom-in-95 duration-500 rounded-[3rem]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {queue.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-6 rounded-3xl bg-secondary/30 border border-border">
                    <div className="flex items-center gap-4">
                      {item.status === 'processing' ? <Loader2 className="animate-spin text-primary" size={18} /> :
                       item.status === 'completed' ? <CheckCircle2 className="text-accent-green" size={18} /> :
                       item.status === 'error' ? <XCircle className="text-destructive" size={18} /> : <Clock className="opacity-30" size={18} />}
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="relative h-12">
              <input type="file" accept=".json" onChange={handleImportJson} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {!history || history.length === 0 ? (
              <div className="col-span-full py-20 text-center opacity-30">
                ...
                <p className="text-xs font-black uppercase tracking-widest">Biblioteca Vazia</p>
              </div>
            ) : (
              history.map((exam: any, idx: number) => (
                <div key={idx} className="flex flex-col gap-3">
                  <div className={`soe-card p-8 rounded-[2.5rem] transition-all ${expandedExam === exam.name ? 'ring-2 ring-primary border-primary/50' : 'hover:border-primary/30'}`}>
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex items-center gap-2">
                         <div className="flex flex-col items-end gap-1 mr-2">
                            <span className="text-[8px] font-black uppercase opacity-30">Integração</span>
                            <button 
                              onClick={() => handleToggleIntegration(exam.name, exam.isIntegrated)}
                              className={`relative w-9 h-5 rounded-full transition-all duration-300 ${exam.isIntegrated ? 'bg-[var(--primary)]' : 'bg-border/50'}`}
                            >
                              <div className={`absolute top-1 w-3 h-3 rounded-full bg-white shadow-sm transition-all duration-300 ${exam.isIntegrated ? 'left-5' : 'left-1'}`} />
                            </button>
                         </div>
                         <button onClick={() => handleExportJson(exam.name)} className="p-2.5 rounded-xl hover:bg-secondary text-muted-foreground transition-colors">
                          <Download size={18} />
                        </button>
                        <button 
                          onClick={async () => {
                            if (confirm("Deseja apagar este arquivo?")) {
                              await deleteFileMutation.mutateAsync({ fileName: exam.name });
                    ...
                    <div className="cursor-pointer" onClick={() => setExpandedExam(expandedExam === exam.name ? null : exam.name)}>
                      <h4 className="text-base font-black truncate leading-tight tracking-tight">{exam.name.replace("questoes_", "").split("_")[0]}</h4>
                      <div className="flex items-center gap-3 mt-3">
                        <span className="text-[10px] font-bold opacity-30">{exam.date}</span>
                        <span className="w-1 h-1 rounded-full bg-border" />
                        <span className="text-[10px] font-black text-primary uppercase tracking-widest">{exam.questionCount} Questões</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {expandedExam === exam.name && (
                    <div className="animate-in slide-in-from-top-2 duration-300 px-4">
                      <div className="soe-card bg-secondary/20 rounded-[2rem] max-h-72 overflow-y-auto p-6 space-y-4 shadow-inner">
                        <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] border-b border-border pb-3">Conteúdo Extraído</p>
                        {exam.questions.map((q: any, qIdx: number) => (
                          <div key={qIdx} className="space-y-1.5 pb-2 border-b border-border/30 last:border-0">
                            <p className="text-[11px] font-medium leading-relaxed opacity-70 line-clamp-2">{q.statement}</p>
                            <span className="text-[9px] font-black text-primary uppercase tracking-widest">{q.subject}</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="soe-card p-8 rounded-[3rem] space-y-6">
              <div className="space-y-2 border-b border-border pb-4">
                <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2">
                  <Target size={18} className="text-primary" /> Seleção de Fontes
                </h3>
                <p className="text-[10px] text-muted-foreground opacity-60">Escolha as provas para análise estratégica.</p>
              </div>
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto no-scrollbar">
                {history?.map((exam) => (
                  <button 
                    key={exam.name}
                    onClick={() => {
                      if (selectedExams.includes(exam.name)) setSelectedExams(selectedExams.filter(e => e !== exam.name));
                  ...
                  >
                    <span className="text-[11px] font-bold truncate max-w-[200px]">{exam.name.replace("questoes_", "").split("_")[0]}</span>
                    {selectedExams.includes(exam.name) && <CheckCircle2 size={14} />}
                  </button>
                ))}
              </div>
            ...
            <div className="soe-card p-8 bg-primary/5 border-primary/10 rounded-[3rem] space-y-4">
              <div className="flex items-center gap-3 text-primary">
                <Info size={18} />
                <h5 className="text-[10px] font-black uppercase tracking-widest">Dica Estratégica</h5>
              </div>
              <p className="text-[11px] opacity-60 leading-relaxed italic">
                "O Raio-X funciona melhor com 3 ou mais provas da mesma banca. Para o Mapeamento de Edital, selecione apenas a prova que deseja analisar a cobertura."
              </p>
            </div>
          </div>
            ...
            <div className="soe-card p-12 rounded-[4rem] min-h-[600px] bg-secondary/5 border-border/50 relative overflow-hidden">
               {!analysisResult && !coverageData && !isAnalyzing && (
                 <div className="absolute inset-0 flex flex-col items-center justify-center opacity-20 text-center p-20 space-y-6">
                    ...
                    <div className="space-y-2">
                       <h4 className="text-sm font-black uppercase tracking-widest">Painel de Inteligência</h4>
                       <p className="text-xs font-medium">Selecione provas e execute uma análise para ver os dados estratégicos.</p>
                    </div>
                 </div>
               )}
               ...
               {isAnalyzing && (
                 <div className="absolute inset-0 flex flex-col items-center justify-center space-y-6 bg-background/50 backdrop-blur-sm z-10">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest animate-pulse">Cruzando Dados com IA...</p>
                 </div>
               )}
               ...
               {coverageData && (
                 <div className="space-y-10 animate-in fade-in zoom-in-95 duration-500">
                    <div className="flex items-center gap-3 border-b border-border pb-6 mb-8">
                       ...
                       <div>
                          <h2 className="text-xl font-black uppercase tracking-tight m-0">Cobertura de Edital</h2>
                          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest m-0 opacity-40">Mapeamento de Conteúdo Programático</p>
                       </div>
                       <div className="ml-auto text-right">
                          <p className="text-4xl font-black text-primary m-0 tabular-nums">{coverageData.coveragePercentage}</p>
                          <p className="text-[9px] font-black uppercase tracking-widest m-0 opacity-40">Taxa de Cobertura</p>
                       </div>
                    </div>

                    <div className="grid grid-cols-1 gap-4">
                       {coverageData.mappedItems?.map((item: any, i: number) => (
                         <div key={i} className="flex items-center justify-between p-6 rounded-3xl bg-secondary/20 border border-border group hover:bg-primary/5 transition-all">
                            <div className="flex items-center gap-4">
      ...
      {analysisResult && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12 animate-in fade-in duration-500">
           <div className="absolute inset-0 bg-background/60 backdrop-blur-3xl" onClick={() => setAnalysisResult(null)} />
           ...
           <div className="relative w-full max-w-5xl max-h-[90vh] bg-secondary/80 border border-white/10 rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col animate-in slide-in-from-bottom-10 duration-700">
              ...
              <div className="px-12 py-10 flex items-center justify-between border-b border-white/5 bg-gradient-to-r from-primary/10 to-transparent">
                 <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                       <div className="flex items-center gap-3">
                          <h2 className="text-3xl font-black uppercase tracking-tight leading-none">Raio-X Estratégico</h2>
                          <span className="px-3 py-1 rounded-full bg-primary/20 text-[8px] font-black uppercase tracking-[0.2em] text-primary border border-primary/20">CONFIDENCIAL</span>
                       </div>
                       <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.4em] opacity-50">Inteligência Competitiva de Dados</p>
                    </div>
                 </div>
                 <button 
                 ...
                 >
                    <XCircle size={24} className="opacity-30 group-hover:opacity-100 transition-opacity" />
                 </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-12 md:px-20 custom-scrollbar bg-gradient-to-b from-transparent to-black/20">
                 <div className="prose prose-invert max-w-none">
                    <div className="text-foreground/80 leading-relaxed font-medium selection:bg-primary/40">
                       <ReactMarkdown 
                        components={{
                          h1: ({node, ...props}) => <h1 className="text-4xl font-black mb-10 border-b border-white/10 pb-4 text-white" {...props} />,
                          h2: ({node, ...props}) => <h2 className="text-2xl font-black mt-12 mb-6 text-primary flex items-center gap-3 before:w-1 before:h-6 before:bg-primary before:rounded-full" {...props} />,
                          h3: ({node, ...props}) => <h3 className="text-lg font-black mt-8 mb-4 text-white/90" {...props} />,
                          p: ({node, ...props}) => <p className="text-sm leading-8 mb-6 opacity-70" {...props} />,
                          table: ({node, ...props}) => (
                          ...
                          th: ({node, ...props}) => <th className="p-5 text-[10px] font-black uppercase tracking-widest text-primary border-b border-white/5" {...props} />,
                          td: ({node, ...props}) => <td className="p-5 text-xs border-b border-white/5 opacity-70" {...props} />,
                          strong: ({node, ...props}) => <strong className="font-black text-primary/90" {...props} />,
                          ul: ({node, ...props}) => <ul className="space-y-4 my-6 list-none p-0" {...props} />,
                          li: ({node, ...props}) => (
                            <li className="flex items-start gap-3 text-sm opacity-70 before:w-1.5 before:h-1.5 before:bg-primary before:rounded-full before:mt-2 before:shrink-0" {...props} />
                          ),
                        }}
                       >
                        {analysisResult}
              ...
              <div className="px-12 py-8 border-t border-white/5 bg-black/40 flex justify-between items-center">
                 <div className="flex items-center gap-8">
                    <div className="space-y-1">
                       <p className="text-[8px] font-black uppercase tracking-widest opacity-30">Gerado pelo Sistema</p>
                       <p className="text-[10px] font-bold tabular-nums">{new Date().toLocaleDateString()} • {new Date().toLocaleTimeString()}</p>
                    </div>
                    <div className="w-px h-8 bg-white/5" />
                    <div className="space-y-1">
                       <p className="text-[8px] font-black uppercase tracking-widest opacity-30">Fontes Analisadas</p>
                       <p className="text-[10px] font-bold">{selectedExams.length} Provas Mineradas</p>
                    </div>
                 </div>
