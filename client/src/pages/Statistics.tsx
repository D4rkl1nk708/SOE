...
export default function Statistics() {
  ...
  const totalQuestions = disciplineStats.reduce(
    (acc: any, d: any) => acc + (d.performance?.questionsResolved || 0),
    0,
  );
  const totalCorrect = disciplineStats.reduce(
    (acc: any, d: any) => acc + (d.performance?.correctCount || 0),
    0,
  );
  const overallAccuracy =
    totalQuestions > 0 ? Math.round((totalCorrect / totalQuestions) * 100) : 0;
  const totalStudyTime = disciplineStats.reduce(
    (acc: any, d: any) => acc + (d.studyTimeSeconds || 0),
    0,
  );
  ...
  const disciplinesByAccuracy = [...disciplineStats]
    .filter((d: any) => d.performance && d.performance.questionsResolved > 0)
    .sort(
      (a: any, b: any) =>
  ...
  const studyTimeData = [...disciplineStats]
    .filter((d: any) => (d.studyTimeSeconds || 0) > 0)
    .sort(
      (a: any, b: any) => (b.studyTimeSeconds || 0) - (a.studyTimeSeconds || 0),
    )
    .slice(0, 7)
    .map((d: any) => ({
  ...
  const accuracyData = disciplinesByAccuracy.slice(0, 8).map((d: any) => ({
    name: d.name.length > 14 ? d.name.slice(0, 13) + "…" : d.name,
  ...
  const CT = ({ active, payload, label }: any) => {
    ...
    return (
      ...
      >
        <p className="font-semibold mb-1" style={{ color: "var(--app-fg)" }}>
          {label}
        </p>
        {payload.map((p: any) => (
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "var(--muted-text)" }}>
            Seu progresso real em números e insights
          </p>
        </div>
          ...
          {!resetConfirm ? (
            <button
              ...
              style={{
                ...
                border:
                  "1px solid color-mix(in srgb, var(--accent-red, #dc2626) 20%, transparent)",
              }}
      ...
      <AnimatePresence>
        {aiInsight && (
          <motion.div
            ...
            style={{
              background:
                "linear-gradient(135deg, color-mix(in srgb, var(--primary) 15%, transparent) 0%, color-mix(in srgb, var(--primary) 5%, transparent) 100%)",
              border:
          ...
          >
            ...
            <Sparkles
              className="absolute -right-2 -bottom-2 w-16 h-16 opacity-10 rotate-12"
              style={{ color: "var(--primary)" }}
            />
          </motion.div>
        )}
          ...
          <div className="soe-card p-5">
            ...
            <div className="space-y-8">
              ...
              >
                ...
                <p
                  className="text-[10px] mt-4 text-center italic"
                  style={{ color: "var(--muted-text)" }}
                >
                  As barras mais escuras representam horários com mais sessões
                  registradas.
                </p>
              </div>
            </div>
              ...
              <div className="h-48 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      content={<CT />}
                      cursor={{ fill: "var(--stat-bg)", opacity: 0.4 }}
                    />
                    <Bar dataKey="horas" radius={[0, 4, 4, 0]} unit="h">
                      {studyTimeData.map((entry: any, index: any) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={entry.color}
                          fillOpacity={0.8}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Bar dataKey="acerto" radius={[4, 4, 0, 0]} unit="%">
                      {accuracyData.map((entry: any, index: any) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={entry.color}
                          fillOpacity={0.8}
        ...
        <div className="space-y-6">
          <div className="soe-card p-5">
            ...
            {examChartData.length > 0 ? (
              ...
              <div
                className="h-40 flex items-center justify-center text-xs text-center p-4"
                style={{ color: "var(--muted-text)" }}
              >
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      boxShadow:
                        "0 0 8px color-mix(in srgb, var(--accent-amber) 40%, transparent)",
                    }}
                  />
                </div>
              </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="soe-card p-5">
            <h3
              ...
              style={{ color: "var(--app-fg)" }}
            >
              <span style={{ color: "var(--accent-green)" }}>↑</span> Melhores
              disciplinas
            </h3>
            <div className="space-y-3">
              {disciplinesByAccuracy.slice(0, 3).map((d: any, i: any) => (
          ...
          <div className="soe-card p-5">
            <h3
              ...
              style={{ color: "var(--app-fg)" }}
            >
              <span style={{ color: "var(--accent-red, #dc2626)" }}>↓</span>{" "}
              Para reforçar
            </h3>
            <div className="space-y-3">
              {[...disciplinesByAccuracy]
                .reverse()
                .slice(0, 3)
                .map((d: any, i: any) => (
                  <div key={d.disciplineId} className="flex items-center gap-3">
      ...
      {disciplineStats.length === 0 && (
        <div className="soe-card p-12 text-center">
          ...
          <p className="text-sm" style={{ color: "var(--muted-text)" }}>
            Cadastre disciplinas e temas para ver suas estatísticas aqui.
          </p>
        </div>
      )}
            ...
            {[...disciplineStats]
              ...
              .map((d: any) => {
                ...
                return (
                  <div
                    key={d.disciplineId}
                    className="flex items-center gap-3 px-4 py-3 rounded-2xl group transition-all hover:scale-[1.01]"
                    style={{
                      background: "var(--stat-bg)",
                      border: "1px solid var(--card-border)",
                    }}
                      ...
                      {acc !== null && (
                        <div className="text-right">
                          ...
                          >
                            <div
                              className="h-full rounded-full"
                              style={{ width: `${acc}%`, background: acColor }}
                            />
                          </div>
                        </div>
                      )}
