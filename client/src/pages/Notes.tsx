...
import { 
  Search, Plus, FileText, Trash2, Upload, Sparkles, 
  ChevronLeft, Bold, Italic, Underline, Highlighter, 
  List, ListOrdered, AlignLeft, AlignCenter, ImagePlus,
  Check, Clock, Download, ChevronRight, X
} from "lucide-react";
...
function RichEditor({ 
  value, onChange, placeholder, onAiAction 
}: { 
  value: string; onChange: (v: string) => void; placeholder?: string; onAiAction: (action: any) => Promise<void> 
}) {
  ...
  const toolbarGroups = [
    [
      ...
      { icon: <Highlighter size={16} />, cmd: "hiliteColor", val: "#fef08a", title: "Destacar" },
    ],
    [
    ...
    [
      ...
      { icon: <ListOrdered size={16} />, cmd: "insertOrderedList", title: "Numerada" },
    ],
    [
      { icon: <AlignLeft size={16} />, cmd: "justifyLeft", title: "Esquerda" },
      { icon: <AlignCenter size={16} />, cmd: "justifyCenter", title: "Centro" },
    ],
  ];
...
export default function Notes() {
  ...
  const generateFlashcardsMut = trpc.ai.generateFlashcardsFromText.useMutation({
    onSuccess: (data) => { toast.success(`${data.count} flashcards criados!`); utils.flashcard.list.invalidate(); },
  ...
  const handleAiAction = async ({ action, text }: { action: string; text: string }) => {
    const { data: stats } = await utils.dashboard.getStats.fetch();
    const apiKey = (stats?.settings as any)?.aiApiKey;
    const provider = (stats?.settings as any)?.aiProvider;

    const res = await utils.client.ai.processText.mutate({ action, text, apiKey, provider });
  ...
  const filteredNotes = notes.filter(n => {
    ...
    const matchesDisc = filterDisciplineId ? n.disciplineId === filterDisciplineId : true;
      ...
      <div className={`${activeNoteId ? "hidden md:flex" : "flex"} flex flex-col w-full md:w-[320px] shrink-0 border-r border-white/5 bg-white/[0.01] backdrop-blur-3xl transition-all`}>
        <div className="p-4 md:p-6 space-y-4 md:space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsCreating(true)} className="w-10 h-10 rounded-xl bg-[var(--primary)] text-[var(--primary-foreground)] shadow-lg shadow-[var(--primary-shadow)] active:scale-95 transition-all flex items-center justify-center">
                    <Plus size={20} />
                </button>
            </div>
        ...
        <div className="p-4 border-t border-white/5 mt-auto">
            <button onClick={() => { setIsCreating(true); importFileRef.current?.click(); }} className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl border-2 border-dashed border-white/5 text-[10px] font-black uppercase tracking-widest opacity-40 hover:opacity-100 transition-all active:scale-[0.98]">
        ...
        {activeNote ? (
            <>
                <div className="px-4 md:px-8 py-3 md:py-4 border-b border-white/5 flex items-center justify-between gap-2 md:gap-6 backdrop-blur-md">
                    <div className="flex-1 flex items-center gap-2 md:gap-4 min-w-0">
                        <button onClick={() => setActiveNoteId(null)} className="md:hidden p-3 rounded-xl bg-white/5 text-[var(--primary)] shrink-0"><ChevronLeft size={20} /></button>
                ...
                <div className="flex-1 overflow-hidden">
                    <RichEditor key={(activeNote as any).id} value={editorContent} onChange={handleEditorChange} onAiAction={handleAiAction} placeholder="Comece a escrever..." />
                </div>
            </>
        ) : (
            <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 text-center">
                ...
                <div className="space-y-4 mb-10 max-w-xs mx-auto">
                    <h2 className="text-2xl md:text-3xl font-black tracking-tight" style={{ color: "var(--app-fg)" }}>Sua Biblioteca Mental</h2>
                    <p className="text-xs md:text-sm opacity-30 leading-relaxed">Organize seus conhecimentos em resumos ricos e deixe a IA transformar tudo em flashcards automaticamente.</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 w-full max-w-[280px] sm:max-w-none justify-center">
                    <button onClick={() => setIsCreating(true)} className="px-8 py-4 rounded-2xl bg-[var(--primary)] text-[var(--primary-foreground)] font-black text-xs uppercase tracking-widest shadow-2xl shadow-[var(--primary-shadow)] hover:brightness-110 active:scale-95 transition-all">
                        + Nova Anotação
                    </button>
                    <button onClick={() => { setIsCreating(true); importFileRef.current?.click(); }} className="px-8 py-4 rounded-2xl bg-white/5 border border-white/10 font-black text-xs uppercase tracking-widest opacity-60 hover:opacity-100 transition-all">
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg">
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <div className="p-6 md:p-10 space-y-8">
                  <div className="flex items-center justify-between">
                      <h2 className="text-2xl font-black tracking-tight" style={{ color: "var(--app-fg)" }}>Novo Documento</h2>
                      <button onClick={() => setIsCreating(false)} className="p-3 rounded-2xl bg-white/5 hover:bg-rose-500/10 text-white/40 hover:text-rose-500 transition-all"><X size={20} /></button>
                  </div>

                  <div className="space-y-6">
                      <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-2">Título do Resumo</label>
                          <input autoFocus value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Ex: Controle de Constitucionalidade"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-2">
                              <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-2">Tema Relacionado</label>
