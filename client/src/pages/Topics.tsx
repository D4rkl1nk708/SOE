    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-sm opacity-60">Organize o conteúdo programático do seu edital.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1 group">
            ...
            <input type="text" placeholder="Pesquisar por nome do tema..."
        ...
        <select value={filters.disciplineId || ""} onChange={e => setFilters({...filters, disciplineId: e.target.value ? Number(e.target.value) : undefined})}
            ...
            {disciplinesData?.map(d => <option key={d.id} value={d.id} className="bg-slate-900">{d.name}</option>)}
        </select>

        {(filters.search || filters.disciplineId) && (
            <button onClick={clearFilters} className="h-14 px-6 rounded-2xl bg-rose-500/5 text-rose-500 border border-rose-500/10 hover:bg-rose-500 hover:text-white transition-all">
                <X size={20} />
            </button>
        )}
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          <div className="py-20 text-center opacity-30 font-black uppercase text-[10px] tracking-widest animate-pulse">Carregando seus temas...</div>
          ...
          <div className="soe-card py-20 flex flex-col items-center justify-center border-dashed opacity-40">
              ...
              <p className="text-xl font-black uppercase tracking-widest">Nenhum tema encontrado</p>
              <p className="text-xs mt-2">Use a barra de pesquisa ou mude os filtros.</p>
          </div>
        ) : (
          <div className="grid gap-4">
            {topicsData?.topics.map(topic => {
              ...
              return (
                <div key={topic.id} className="soe-card group hover:border-[var(--primary-border)] transition-all overflow-hidden relative">
                  <div className="absolute top-0 left-0 w-1 h-full opacity-20" style={{ backgroundColor: discipline?.color }} />
                    ...
                    <div className="flex items-start gap-5">
                        ...
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                <span className="text-[9px] font-black uppercase tracking-widest" style={{ color: discipline?.color }}>{discipline?.name}</span>
                                <div className="w-1 h-1 rounded-full bg-white/10" />
                                <span className="text-[9px] font-black uppercase tracking-widest opacity-30">Tema #{topic.id}</span>
                            </div>
                            <h4 className="text-lg font-black tracking-tight leading-tight" style={{ color: "var(--app-fg)" }}>{topic.name}</h4>
                            
                            <div className="flex items-center gap-4 mt-2">
                                <div className="flex items-center gap-1.5 opacity-40">
                                    <Clock size={12} />
                                    <span className="text-[10px] font-bold">{formatStudyTime((topic as any).studyTimeSeconds || 0)}</span>
                                </div>
                                <div className="flex items-center gap-1.5 opacity-40">
                                    <BarChart2 size={12} />
                                    <span className="text-[10px] font-bold">{(topic as any).performance?.questionsResolved || 0} questões</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 pt-4 md:pt-0 border-white/5">
                        {accuracy > 0 && (
                            <div className="text-right">
                                <p className="text-[9px] font-black uppercase tracking-widest opacity-30 mb-1">Aproveitamento</p>
                                <p className="text-xl font-black" style={{ 
                                    color: accuracy >= 70 ? 'var(--accent-green)' : accuracy >= 50 ? 'var(--accent-amber)' : 'var(--accent-red)'
                                }}>{accuracy}%</p>
                            </div>
                        )}
                        
                        <div className="flex gap-2">
                            <button onClick={() => setEditingTopic({ id: topic.id, name: topic.name, disciplineId: topic.disciplineId, notes: topic.notes || "", studyTimeSeconds: (topic as any).studyTimeSeconds || 0 })}
                            ...
                            <AlertDialog>
                                ...
                                <AlertDialogContent className="rounded-[2.5rem] border-white/10 bg-[var(--app-bg)] p-8">
                                    <AlertDialogHeader>
                                        <AlertDialogTitle className="text-2xl font-black">Excluir Tema?</AlertDialogTitle>
                                        <AlertDialogDescription className="text-sm opacity-60 mt-2">Isto removerá permanentemente o tema "{topic.name}" e todo o histórico de questões vinculado a ele.</AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter className="mt-8 gap-3">
                                        ...
                                        <AlertDialogAction onClick={() => deleteMutation.mutate({ id: topic.id })} className="h-12 rounded-2xl bg-rose-500 text-white font-black uppercase text-[10px] tracking-widest px-8 shadow-xl shadow-rose-500/20">Excluir Tema</AlertDialogAction>
                                    </AlertDialogFooter>
                                </AlertDialogContent>
                            </AlertDialog>
                        </div>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="rounded-[2.5rem] border-white/10 bg-[var(--app-bg)] p-8 max-w-lg">
          <DialogHeader>
              <DialogTitle className="text-2xl font-black">Novo Tema de Estudo</DialogTitle>
              <DialogDescription className="text-sm opacity-60">Registre um novo assunto no seu cronograma.</DialogDescription>
          </DialogHeader>
          <div className="space-y-6 py-6">
             <div className="space-y-2">
                 <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Disciplina Relacionada</label>
                 <select onChange={val => setFormData({...formData, disciplineId: Number(val.target.value)})}
                   ...
                   {disciplinesData?.map(d => <SelectItem key={d.id} value={String(d.id)}>{d.name}</SelectItem>)}
                 </select>
             </div>
             <div className="space-y-2">
                 <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Título do Assunto</label>
                 <input placeholder="Ex: Controle de Constitucionalidade" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})}
                        className="w-full px-5 h-14 rounded-2xl bg-white/5 border border-white/5 text-sm font-bold outline-none focus:border-[var(--primary)] transition-all" />
             </div>
             <button onClick={handleCreate} className="w-full h-14 rounded-2xl bg-[var(--primary)] text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-[var(--primary-shadow)] active:scale-[0.98] transition-all">
                 Salvar no Edital
             </button>
          </div>
        </DialogContent>
      ...
      <Dialog open={!!editingTopic} onOpenChange={o => !o && setEditingTopic(null)}>
        <DialogContent className="rounded-[2.5rem] border-white/10 bg-[var(--app-bg)] p-8 max-w-lg">
          <DialogHeader>
              <DialogTitle className="text-2xl font-black">Editar Tema</DialogTitle>
          </DialogHeader>
          {editingTopic && (
            <div className="space-y-6 py-6">
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Nome do Tema</label>
                    <input value={editingTopic.name} onChange={e => setEditingTopic({...editingTopic, name: e.target.value})}
                           className="w-full px-5 h-14 rounded-2xl bg-white/5 border border-white/5 text-sm font-bold outline-none focus:border-[var(--primary)] transition-all" />
                </div>
                <button onClick={handleUpdate} className="w-full h-14 rounded-2xl bg-[var(--primary)] text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-[var(--primary-shadow)]">
                    Salvar Alterações
                </button>
            </div>
          )}
        ...
        <DialogContent className="rounded-[2.5rem] border-white/10 bg-[var(--app-bg)] p-8 max-w-xl">
          <DialogHeader>
              <div className="flex items-center gap-3 mb-2">
                  ...
                  <DialogTitle className="text-2xl font-black">Briefing de Pré-Estudo</DialogTitle>
              </div>
              <DialogDescription className="text-sm opacity-60">Para otimizar sua retenção, escreva em 2 ou 3 frases o que você já sabe sobre este tema antes de começar.</DialogDescription>
          </DialogHeader>
          <textarea placeholder="Seja breve... (Ex: 'É o controle feito pelo STF sobre as leis...')" 
                    value={preTestText} onChange={e => setPreTestText(e.target.value)} rows={6} 
          ...
          <DialogFooter className="mt-8">
              <button onClick={doCreate} className="w-full h-14 rounded-2xl bg-[var(--primary)] text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-[var(--primary-shadow)]">
