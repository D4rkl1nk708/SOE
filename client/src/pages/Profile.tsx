...
import { 
  ...
  Cpu, Layout, Smartphone, Database, Zap, ShieldCheck, Camera
} from "lucide-react";
    ...
    <div className="space-y-10 py-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="space-y-2">
            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-primary">Visual & Interface</h3>
            <p className="text-[10px] font-bold text-white/30 uppercase leading-relaxed">Customize a experiência visual do seu ecossistema SOE.</p>
        </div>
        <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {(["light", "dark"] as const).map(m => (
                <button key={m} onClick={() => { if (theme !== m) toggleTheme(); }}
                  ...
                  <div className={`p-3 rounded-2xl ${theme === m ? 'bg-primary text-white' : 'bg-white/10 text-white/40'}`}>
                    {m === "light" ? <Sun size={20} /> : <Moon size={20} />}
                  </div>
                  <div className="text-left">
                    <p className={`font-black text-[10px] uppercase tracking-widest ${theme === m ? 'text-white' : 'text-white/40'}`}>
            ...
            <div className="p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-6">
                <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40">Esquema de Cores</p>
                  ...
                  {(Object.entries(COLOR_THEMES) as [ColorTheme, any][]).map(([key, cfg]) => {
                    ...
                    return (
                      <button key={key} onClick={() => setColorTheme(key)}
                        ...
                        <div className="w-8 h-8 rounded-full border-2 border-background shadow-xl" style={{ background: color }} />
                        <span className="text-[8px] font-black uppercase tracking-widest truncate">{cfg.label}</span>
                      </button>
                    );
                  })}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="space-y-2">
            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-primary">Inteligência Artificial</h3>
            <p className="text-[10px] font-bold text-white/30 uppercase leading-relaxed">Gerencie os motores que alimentam o Mentor e a Mineração.</p>
        </div>
        <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                    <label className="text-[9px] font-black uppercase tracking-widest opacity-30 ml-1">Provedor</label>
            ...
            <div className="space-y-3">
                <div className="flex items-center justify-between ml-1">
                    <label className="text-[9px] font-black uppercase tracking-widest opacity-30">Chaves de API</label>
                ...
                <textarea value={apiKey} onChange={(e) => setApiKey(e.target.value)}
                          ...
                          rows={4} className="w-full px-6 py-5 rounded-[2rem] bg-white/5 border border-white/5 text-[11px] font-mono outline-none resize-none focus:border-primary transition-all custom-scrollbar" />
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="space-y-2">
            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-primary">Sistema & Backup</h3>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <label className={`flex items-center justify-between p-6 rounded-[2rem] border transition-all cursor-pointer ${autoBackupEnabled ? 'bg-primary/5 border-primary' : 'bg-white/5 border-white/5 opacity-50'}`}>
                <div className="flex items-center gap-4">
                    ...
                    <div>
                      <span className="text-xs font-black uppercase tracking-widest">Backup em Tempo Real</span>
                      <p className="text-[9px] font-bold opacity-30 uppercase">Sincroniza com Google Drive / Dropbox</p>
                    </div>
                </div>
                <input type="checkbox" checked={autoBackupEnabled} onChange={(e) => setAutoBackupEnabled(e.target.checked)} className="hidden" />
                <div className={`w-12 h-6 rounded-full p-1 transition-all ${autoBackupEnabled ? 'bg-primary' : 'bg-white/10'}`}>
                    <div className={`w-4 h-4 rounded-full bg-white transition-all ${autoBackupEnabled ? 'ml-6' : 'ml-0'}`} />
                </div>
            </label>
            ...
            {autoBackupEnabled && (
                <div className="flex gap-3 animate-in slide-in-from-left-4 duration-300">
                    ...
                    <button onClick={handleSaveSync} className="px-8 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 text-[9px] font-black uppercase tracking-widest transition-all">
                        Salvar
                    </button>
                </div>
            )}
...
function DiarioOficialTab() {
  ...
  if (loading) return <div className="p-20 text-center opacity-20 font-black uppercase text-[10px] tracking-[0.5em] animate-pulse">Sincronizando Sentinela...</div>;
  ...
  return (
    <div className="space-y-12 py-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="space-y-2">
            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-primary">Configuração de Busca</h3>
            <p className="text-[10px] font-bold text-white/30 uppercase leading-relaxed">O SOE monitora automaticamente as publicações da União em busca de citações ao seu nome.</p>
        </div>
        <div className="lg:col-span-2 space-y-8">
            <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 space-y-8 relative overflow-hidden">
                ...
                <div className="space-y-4">
                    <label className="text-[9px] font-black uppercase tracking-widest opacity-30 ml-1">Nome Completo para Monitoramento</label>
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <button onClick={handleSave} className="px-10 h-14 rounded-2xl bg-primary text-white font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-primary/20 active:scale-95 transition-all">
                            Ativar
                        </button>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between ml-1">
                        <label className="text-[9px] font-black uppercase tracking-widest opacity-30">Ciclo de Varredura</label>
                        <span className="text-[10px] font-black uppercase tracking-widest text-primary">{intervalMinutes < 60 ? `${intervalMinutes}m` : `${intervalMinutes/60}h`}</span>
                    </div>
      ...
      {saved && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="space-y-2">
                <h3 className="font-black text-xs uppercase tracking-[0.3em] text-primary">Status em Tempo Real</h3>
                <p className="text-[10px] font-bold text-white/30 uppercase leading-relaxed">Resultado da última varredura automática realizada pelo sistema.</p>
            </div>
            <div className="lg:col-span-2 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-2">
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-30 text-primary">Último Checkpoint</p>
                        <p className="text-xl font-black">{lastCheck ? new Date(lastCheck).toLocaleString() : "Aguardando..."}</p>
                    </div>
                    <div className={`p-8 rounded-[2rem] border transition-all flex flex-col justify-center ${lastResult?.newCount ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/[0.02] border-white/5'}`}>
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-30">Ocorrências</p>
                        <div className="flex items-center justify-between">
                            <p className="text-xl font-black">{lastResult ? (lastResult.total === 0 ? "Nenhuma" : lastResult.total) : "---"}</p>
                            {lastResult?.newCount ? <span className="px-3 py-1 rounded-full bg-emerald-500 text-white font-black text-[8px] uppercase tracking-widest animate-pulse">Novo!</span> : null}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex gap-4">
                    <button onClick={handleCheckNow} disabled={checking}
                        ...
                        {checking ? <Loader2 size={20} className="animate-spin" /> : <Zap size={20} />}
                        {checking ? "Varrendo Diários..." : "Forçar Verificação Agora"}
                    </button>
...
export default function Profile() {
  ...
  const TABS = [
    ...
    { id: "history" as Tab,    label: "Linha do Tempo", icon: HistoryIcon, desc: "Progresso" },
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-8 bg-secondary/40 backdrop-blur-md border border-white/5 p-10 rounded-[3rem] overflow-hidden">
          ...
          <div className="flex items-center gap-8">
            <div className="relative group/avatar cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                ...
                <div className="relative w-24 h-24 rounded-[2.5rem] bg-gradient-to-br from-primary to-purple-600 p-[2px] shadow-2xl group-hover/avatar:scale-105 transition-transform duration-500"
                  ...
                  <div className="w-full h-full rounded-[2.4rem] overflow-hidden bg-secondary flex items-center justify-center border border-white/10">
                    {stats?.settings?.profileImage ? (
                      <img src={stats.settings.profileImage} alt="Profile" className="w-full h-full object-cover" />
                    ) : (
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-3">
                  <span className="px-3 py-1 rounded-full bg-[var(--accent-green)]/10 text-[var(--accent-green)] text-[9px] font-black uppercase tracking-widest border border-[var(--accent-green)]/20">Membro Premium</span>
                  <span className="text-[10px] font-bold opacity-30 uppercase tracking-widest">ID: {stats?.userId?.slice(0,8) || 'SOE-USER'}</span>
              </div>
            </div>
          </div>

          <button onClick={logout} className="flex items-center justify-center gap-3 px-8 h-14 rounded-2xl bg-white/5 hover:bg-rose-500/10 text-white/40 hover:text-rose-500 border border-white/5 hover:border-rose-500/20 transition-all active:scale-95 font-black text-[10px] uppercase tracking-widest group">
          ...
          return (
            <button key={t.id} onClick={() => { setTab(t.id); window.location.hash = t.id; }}
              ...
              <div className="text-left hidden sm:block">
                <p className="font-black text-[10px] uppercase tracking-widest leading-none">{t.label}</p>
                <p className={`text-[8px] mt-1 hidden md:block ${active ? 'text-white/60' : 'opacity-30'}`}>{t.desc}</p>
              </div>
            </button>
          );
        })}
