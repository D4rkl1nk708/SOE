...
export default function MentorSession() {
  ...
  const recentHits = history.slice(-4).filter((h: any) => h.correct).length;
  ...
  const endSession = (h: SessionEntry[]) => {
    const correct = h.filter((e: any) => e.correct).length;
    const wrong = h.filter((e: any) => !e.correct).length;
  ...
  const accuracy =
    history.length > 0
      ? Math.round(
          (history.filter((h: any) => h.correct).length / history.length) * 100,
        )
      : 0;
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const name =
                          disciplines?.find((d: any) => d.id === id)?.name ??
                          "";
        ...
        {phase === "question" && (
          ...
          >
            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
              <motion.div
                className="h-full bg-gradient-to-r from-[var(--primary)] to-[var(--accent-amber)] shadow-[0_0_15px_var(--primary-shadow)]"
                initial={{ width: 0 }}
                animate={{ width: `${(history.length / SESSION_SIZE) * 100}%` }}
                  ...
                  <div className="grid gap-3">
                    {currentQuestion.alternatives.map((alt: any) => {
                      ...
                      return (
                        ...
                        >
                          <span className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-white/5 flex items-center justify-center font-black text-xs shrink-0">
                            {alt.letter}
                          </span>
                    ...
                    <div className="pt-4">
                      {showHint ? (
                        ...
                        <button
                          ...
                          className="text-[10px] font-black uppercase tracking-widest text-amber-500/60 hover:text-amber-500 transition-colors flex items-center gap-1.5 ml-1"
                        >
                  ...
                  <div className="pt-6">
                    {!confirmed ? (
                      <Button
                        ...
                        className="w-full py-6 md:py-5 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest shadow-xl shadow-[var(--primary-shadow)]"
                      >
                        Confirmar Resposta{" "}
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                        <div
                          className={`flex-1 p-4 md:p-5 rounded-xl md:rounded-2xl border flex items-center gap-3 md:gap-4 ${selectedAnswer === currentQuestion.correctAnswer ? "bg-[var(--accent-green)]/10 border-[var(--accent-green)]/20 text-[var(--accent-green)]" : "bg-rose-500/10 border-rose-500/20 text-rose-500"}`}
                        >
                        ...
                        <Button
                          ...
                          className="w-full sm:w-auto px-10 py-5 md:py-0 rounded-xl md:rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] md:text-xs font-black uppercase tracking-widest transition-all"
                        >
        ...
        {phase === "fixation" && diagnosis && (
          ...
          >
            <div className="soe-card p-6 md:p-10 border-l-4 border-rose-500 relative overflow-hidden">
              ...
              <p
                className="text-base md:text-lg font-medium leading-relaxed mb-6 md:mb-8 opacity-80"
                style={{ color: "var(--app-fg)" }}
              >
                {diagnosis.diagnosis}
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                ...
                <div className="p-4 md:p-5 rounded-xl md:rounded-2xl bg-[var(--primary-bg-subtle)] border border-[var(--primary-border)] space-y-1 md:space-y-2">
                  <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-[var(--primary)] opacity-60">
                    Regra de Ouro
                  </p>
                  <p className="text-xs md:text-sm font-black text-[var(--primary)]">
                    {diagnosis.rule}
                  </p>
                </div>
              </div>
            ...
            <div className="soe-card p-10 space-y-8">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest opacity-40">
                  {fixationIndex + 1} / {diagnosis.fixationQuestions.length}
                </span>
              </div>
              ...
              <div className="pt-4">
                {!fixationConfirmed ? (
                  <Button
                    onClick={() => setFixationConfirmed(true)}
                    disabled={!fixationAnswer}
                  ...
                  <Button
                    onClick={nextAfterFixation}
                    className="w-full py-4 rounded-2xl bg-[var(--primary)] text-[var(--primary-foreground)] font-black text-xs uppercase tracking-widest shadow-xl shadow-[var(--primary-shadow)]"
                  >
                    {fixationIndex < diagnosis.fixationQuestions.length - 1
                      ? "Próxima Questão"
                      : "Retomar Sessão Principal"}
                  </Button>
                )}
              </div>
            </div>
            ...
            <div className="soe-card p-12 text-center relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-6">
                <div className="inline-block p-8 rounded-[3rem] bg-white/[0.02] border border-white/5 shadow-2xl relative">
                  ...
                  <Sparkles
                    size={24}
                    className="absolute top-4 right-4 text-[var(--primary)] animate-pulse"
                  />
                </div>
                ...
                <div className="grid grid-cols-3 gap-6 max-w-md mx-auto">
                  <div className="space-y-1">
                    <p className="text-3xl font-black text-[var(--accent-green)]">
                      {history.filter((h: any) => h.correct).length}
                    </p>
                    <p className="text-[9px] font-black uppercase tracking-widest opacity-40">
                      Acertos
                    </p>
                ...
                <div className="pt-8 flex gap-4">
                  ...
                  <Button
                    onClick={() => navigate("/")}
                    className="flex-1 py-4 rounded-2xl bg-[var(--primary)] text-[var(--primary-foreground)] font-black text-xs uppercase tracking-widest shadow-xl shadow-[var(--primary-shadow)]"
                  >
                    Concluir Relatório
                  </Button>
                </div>
