...
export function ShareProgress({ onClose }: ShareProgressProps) {
  ...
  const generateImage = async () => {
    ...
    if (template === "modern") {
      ...
      roundRect(ctx, 64, 64, 80, 80, 24); ctx.fill();
      ctx.fillStyle = "white";
      ...
      ctx.fillText(new Date().toLocaleDateString("pt-BR", { day: "numeric", month: "long", year: "numeric" }), 64, 210);

      // Main Circle
      const hX = W / 2, hY = 480, hR = 200;
      ctx.beginPath(); ctx.arc(hX, hY, hR, 0, Math.PI * 2);
      ctx.strokeStyle = isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)";
      ctx.lineWidth = 30; ctx.stroke();
      
      if (accuracy > 0) {
        ctx.beginPath(); ctx.arc(hX, hY, hR, -Math.PI/2, (-Math.PI/2) + (accuracy/100)*Math.PI*2);
        ctx.strokeStyle = primary;
        ctx.lineWidth = 30; ctx.lineCap = "round"; ctx.stroke();
      }
      ...
      // Cards
      const cards = [
        { l: "Tempo de Estudo", v: hours > 0 ? `${hours}h ${mins}m` : `${mins}m`, i: "⏱️" },
        { l: "Questões Hoje", v: String(totalQ), i: "📝" },
        { l: "Disciplinas", v: String(disciplines.length), i: "📚" },
        { l: "Nesta Semana", v: weekly?.thisWeek?.questions ? `${weekly.thisWeek.questions} q` : "0 q", i: "📅" }
      ];

      const cW = (W - 128 - 40) / 2, cH = 240;
      cards.forEach((c, idx) => {
        const col = idx % 2, row = Math.floor(idx / 2);
        const x = 64 + col * (cW + 40), y = 780 + row * (cH + 40);
        drawGlassCard(ctx, x, y, cW, cH, 32, isDark ? "rgba(30,41,59,0.5)" : "rgba(255,255,255,0.8)", isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.05)");
      ...
      items.forEach((item, i) => {
        ...
        roundRect(ctx, 64, y, W - 128, 130, 24); ctx.fill();
        ctx.fillStyle = primary;
        ctx.font = "bold 64px system-ui";
      ...
      ctx.beginPath();
      ctx.moveTo(0, H * 0.5); ctx.lineTo(W, H * 0.3); ctx.lineTo(W, H); ctx.lineTo(0, H);
      ctx.fill();
      ...
      const bW = (W - 128 - 40) / 2, bH = 240;
      [{ l: "QUESTÕES", v: String(totalQ) }, { l: "HORAS", v: `${hours}h` }].forEach((b, i) => {
        const x = 64 + i * (bW + 40);
        ctx.fillStyle = "white";
        roundRect(ctx, x, 780, bW, bH, 40); ctx.fill();
        ctx.fillStyle = primary;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col bg-slate-900 border border-white/10" style={{ maxHeight: "95vh" }}>
        
        <div className="flex items-center justify-between px-6 py-4 bg-white/5 border-b border-white/10">
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-amber-400" />
            <h2 className="font-black text-lg text-white">Compartilhar Progresso</h2>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6">
          ...
          <div className="rounded-2xl overflow-hidden bg-black/40 border border-white/5 relative min-h-[300px] flex items-center justify-center">
            {generating ? (
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Criando Obra de Arte...</p>
              </div>
            ) : imageUrl ? (
          ...
          <div className="grid grid-cols-3 gap-3">
            <button onClick={() => { setImageUrl(null); setTimeout(() => generateImage(), 50); }}
              ...
              <span className="text-[10px] font-black uppercase">Atualizar</span>
            </button>
