...
import { Search, X, BookOpen, RefreshCw, ListChecks, ChevronRight } from "lucide-react";
...
export function GlobalSearch() {
  ...
  const { data: stats } = trpc.dashboard.getStats.useQuery(undefined, { enabled: open });
  const { data: revisionsData } = trpc.revision.list.useQuery({ completed: false }, { enabled: open });
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    for (const d of stats.disciplineStats ?? []) {
      if (match(d.name)) {
        out.push({ type: "discipline", id: d.disciplineId, title: d.name, subtitle: `${d.topicCount ?? 0} temas`, href: "/disciplines", color: d.color });
      }
    }
    ...
    for (const d of stats.disciplineStats ?? []) {
      for (const t of d.topics ?? []) {
        if (match(t.name) || match(d.name)) {
          const acc = t.performance && (t.performance.correctCount + t.performance.errorCount) > 0
            ? Math.round(t.performance.correctCount / (t.performance.correctCount + t.performance.errorCount) * 100)
            : null;
          out.push({
            type: "topic", id: t.id, title: t.name,
    ...
    for (const rev of revisionsData ?? []) {
      const topicName = (stats.disciplineStats ?? [])
        .flatMap(d => d.topics ?? [])
        .find(t => t.id === rev.topicId)?.name ?? "Tema";
      const label = rev.type === "test" ? "Teste" : `Revisão ${rev.revisionNumber}`;
      if (match(topicName) || match(label)) {
        out.push({
          type: "revision", id: rev.id,
          title: topicName,
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3.5" style={{ borderBottom: "1px solid var(--card-border)" }}>
          <Search className="w-4 h-4 flex-shrink-0" style={{ color: "var(--muted-text)" }} />
          <input
            ref={inputRef}
            value={query}
          ...
          />
          <button onClick={() => setOpen(false)} className="p-1 rounded-lg hover:opacity-60" style={{ color: "var(--muted-text)" }}>
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {results.length > 0 ? (
              ...
              <li key={`${r.type}-${r.id}-${i}`}>
                ...
                >
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                    style={{ background: r.color ? `${r.color}22` : "var(--stat-bg)", color: r.color || "var(--muted-text)" }}>
                    {iconFor(r.type)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{r.title}</p>
                    <p className="text-xs truncate" style={{ color: "var(--muted-text)" }}>{r.subtitle}</p>
                  </div>
          ...
          <div className="px-4 py-8 text-center text-sm" style={{ color: "var(--muted-text)" }}>
            Nenhum resultado para "<strong>{query}</strong>"
          </div>
        ) : (
          <div className="px-4 py-5 text-center text-xs" style={{ color: "var(--muted-text)" }}>
            Digite para buscar · <kbd className="px-1.5 py-0.5 rounded text-[10px]"
              style={{ background: "var(--stat-bg)", border: "1px solid var(--card-border)" }}>Esc</kbd> para fechar
          </div>
        )}
      </div>
    </div>
...
export function SearchButton() {
  ...
  return (
    ...
    >
      ...
      <kbd className="hidden lg:inline text-[10px] px-1.5 py-0.5 rounded"
