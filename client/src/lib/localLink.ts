...
> = {
  "auth.me": () => localAuthMe(),
  "auth.updateSettings": (i) => localUpdateSettings(i as Record<string, unknown>),
  "auth.logout": () => Promise.resolve({ success: true }),
  "exam.list": () => localExamList(),
  "exam.upsert": (i) => localExamUpsert(i as { id?: string; name: string; date: string }),
  "exam.remove": (i) => localExamRemove(i as { id: string }),
  "discipline.list": () => localDisciplineList(),
  "discipline.create": (i) => localDisciplineCreate(i as { name: string; color: string; weight: number }),
  "discipline.update": (i) => localDisciplineUpdate(i as { id: number; name?: string; color?: string; weight?: number }),
  "discipline.delete": (i) => localDisciplineDelete(i as { id: number }),
  "discipline.reorder": (i) => localDisciplineReorder(i as { orderedIds: number[] }),
  "topic.list": (i) => localTopicList(i as { disciplineId?: number; search?: string }),
  "topic.create": (i) => localTopicCreate(i as { name: string; disciplineId: number; studyDate?: string; notes?: string }),
  "topic.delete": (i) => localTopicDelete(i as { id: number }),
  "topic.update": (i) => localTopicUpdate(i as { id: number; name?: string; disciplineId?: number; notes?: string }),
  "topic.setPerformance": (i) => localTopicSetPerformance(i as { topicId: number; correctCount: number; errorCount: number }),
  "topic.reorder": (i) => localTopicReorder(i as { disciplineId: number; orderedIds: number[] }),
  "topic.resetAllStats": () => localTopicResetAllStats(),
  "revision.list": (i) => localRevisionList(i as { completed?: boolean; ignored?: boolean }),
  "revision.markCompleted": (i) => localRevisionMarkCompleted(i as { id: number; completed: boolean }),
  "revision.markIgnored": (i) => localRevisionMarkIgnored(i as { id: number; ignored: boolean }),
  "import.exportBackup": () => localImportExportBackup(),
  "import.importBackup": (i) => localImportImportBackup(i as { json: string }),
  "import.tecConcursos": () => Promise.reject(new Error("Importação TEC disponível apenas na versão web. Use Exportar/Importar Banco para backup.")),
  "import.tecConcursosScrape": () => Promise.reject(new Error("Importação TEC via URL disponível apenas na versão web. Use a importação XLSX para o modo offline.")),
  "import.generatePushToken": () => Promise.reject(new Error("Token de Push disponível apenas na versão web.")),
  ...
  "mockExam.create": (i) => localMockExamCreate(i as { name: string; date: string; correct: number; wrong: number; blank: number; totalQuestions: number }),
  "note.list": () => localNoteList(),
  "note.upsert": (i) => localNoteUpsert(i as { id?: number; userId: number; disciplineId: number; topicId?: number; title: string; content: string }),
  "note.delete": (i) => localNoteDelete(i as { id: number }),
  "mockExam.update": (i) => localMockExamUpdate(i as { id: number; name?: string; date?: string; correct?: number; wrong?: number; blank?: number; totalQuestions?: number }),
  ...
  return () => ({ op, next }) => {
    ...
    if (!handler) {
      return observable((observer) => {
        observer.error(
          TRPCClientError.from(new Error(`Procedimento local não implementado: ${op.path}`))
        );
      });
    }
    return observable((observer) => {
