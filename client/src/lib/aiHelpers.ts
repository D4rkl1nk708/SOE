...
export function extractJSON(text: string): any {
  ...
  const first = (start !== -1 && startArr !== -1) ? Math.min(start, startArr) : (start !== -1 ? start : startArr);
  ...
  for (let i = 0; i < 20; i++) {
    try {
      ...
      const ob = (attempt.match(/\{/g) || []).length, cb = (attempt.match(/\}/g) || []).length;
      const ok = (attempt.match(/\[/g) || []).length, ck = (attempt.match(/\]/g) || []).length;
      if (ok > ck) attempt += "]".repeat(ok - ck);
      if (ob > cb) attempt += "}".repeat(ob - cb);
      return JSON.parse(attempt);
    } catch (e) {
      const last = Math.max(current.lastIndexOf(","), current.lastIndexOf("["), current.lastIndexOf("{"), current.lastIndexOf(":"));
      if (last <= 0) break;
      current = current.substring(0, last).trim().replace(/[,:]+$/, "").trim();
    }
  }
