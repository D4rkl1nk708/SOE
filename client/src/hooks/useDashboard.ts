...
export function useDragReorder(initialStats: DisciplineStat[]) {
  ...
  useEffect(() => {
    // Only update if the content has actually changed or the length is different
    // This prevents infinite loops if the parent passes a new array reference with same data
    if (JSON.stringify(initialStats) !== JSON.stringify(orderedStats)) {
