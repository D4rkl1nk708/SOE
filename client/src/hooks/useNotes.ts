...
export function useFileImport() {
    ...
    async (
      ...
      try {
        ...
        if (file.name.endsWith(".docx") || file.name.endsWith(".doc")) {
          ...
          toast.success(
            "Arquivo Word importado! Preencha título e disciplina.",
          );
  ...
  const handleImportGdocs = useCallback(async (url: string) => {
    if (!url.trim()) {
      toast.error("Cole a URL do Google Docs.");
      return;
    }
    setIsImporting(true);
    try {
      ...
      toast.error(
        "Erro ao importar Google Doc: " +
          (err instanceof Error
            ? err.message
            : "verifique as permissões do documento"),
      );
    } finally {
      setIsImporting(false);
    }
...
export function useNoteFilters(notes: Note[]) {
  ...
  const filteredNotes = useMemo(() => {
    ...
    if (filterDisciplineId)
      list = list.filter((n) => n.disciplineId === filterDisciplineId);
    if (search) {
...
export function useNotes() {
  ...
  const handleCreate = useCallback(
    async (
      ...
      onReset();
      toast.success(
        content ? "Documento importado com sucesso!" : "Documento criado!",
      );
    },
    [upsertNote, utils],
  );
