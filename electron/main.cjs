...
ipcMain.on("open-tec-browser", (event, token) => {
  ...
  tecWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: false, // Ensures fetch interceptions work locally in the page scope
      nodeIntegration: true, // Necessário para o require('electron') no preload quando contextIsolation é false
    },
  });
...
ipcMain.on("soe-tec-message", async (event, data) => {
  ...
  try {
    ...
    console.error("[SOE][IPC] Erro na requisição interna para o backend:", endpoint, error);
    if (event.sender && !event.sender.isDestroyed()) {
...
// Comportamento de fechar: vai para bandeja ou encerra dependendo da preferência do usuário
app.on("window-all-closed", () => {
  ...
  // minimizeToTray=true → app continua na bandeja, servidor Express continua rodando
});
